...
const checkRegister = () => {
  ...
  let change = Number((cashValue - price).toFixed(2));
  ...
  if (cashValue < price) {
    ...
    displayChangeDue.innerText = "Status: CUSTOMER_INSUFFICIENT_CASH";
    return;
  ...
  if (remainingCid === 0) {
    displayChangeDue.innerHTML = "Status: <b>CLOSED<b> <br><br>" + changeArr.map(cash => `${cash[0]}: $${cash[1].toFixed(2)} <br>`).join(" ");
...
const displayCashInDrawer = () => {
  displayCid.innerHTML = "<h4>Cash in Drawer:</h4>" + cid.map(cash => `<b>${cash[0]}</b>: $${cash[1].toFixed(2)} <br>`).reverse().join("");
}
